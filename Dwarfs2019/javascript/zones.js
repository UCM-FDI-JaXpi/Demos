...
const bufGold = renderBuf(3 /* Inline.B_SCALE */ * 8, 3 /* Inline.B_SCALE */ * 9, canvas => {
    for (let y = 0; y < B_GOLD.length; ++y) {
        for (let x = 0; x < 8; ++x) {
            const n = B_GOLD[y] >> 2 * (7 - x) & 0b11;
            if (n) {
                canvas.fillStyle = '#' + PAL_GOLD[n];
                canvas.fillRect(3 /* Inline.B_SCALE */ * x, 3 /* Inline.B_SCALE */ * y, 3 /* Inline.B_SCALE */, 3 /* Inline.B_SCALE */);
            }
        }
    }
});
...
const bufFactory = renderBuf(3 /* Inline.B_SCALE */ * 24, 3 /* Inline.B_SCALE */ * 27, canvas => {
    for (let a of FACTORY) {
        if (typeof a == 'number') {
            canvas.fillStyle = '#' + PAL_WASTELAND[a];
        }
        else {
            canvas.fillRect(3 /* Inline.B_SCALE */ * a[0], 3 /* Inline.B_SCALE */ * a[1], 3 /* Inline.B_SCALE */ * a[2], 3 /* Inline.B_SCALE */ * a[3]);
        }
    }
    for (let n = 0; n < 6; ++n) {
        canvas.fillRect(3 /* Inline.B_SCALE */ * 7, 3 /* Inline.B_SCALE */ * (2 * n + 7), 3 /* Inline.B_SCALE */, 3 /* Inline.B_SCALE */);
        canvas.fillRect(3 /* Inline.B_SCALE */ * 16, 3 /* Inline.B_SCALE */ * (2 * n + 7), 3 /* Inline.B_SCALE */, 3 /* Inline.B_SCALE */);
    }
});
...
const bufExit = renderBuf(3 /* Inline.B_SCALE */ * 13, 3 /* Inline.B_SCALE */ * 13, canvas => {
    ...
    for (let y = 0; y < B_PORTAL.length; ++y) {
        for (let x = 0; x < 6; ++x) {
            ...
            if (n != 0b00) {
                ...
                canvas.fillRect(3 /* Inline.B_SCALE */ * x + x0, 3 /* Inline.B_SCALE */ * y + y0, 3 /* Inline.B_SCALE */, 3 /* Inline.B_SCALE */);
            }
        }
    }
}
...
function renderDwarfs(t, canvas, palette, zonePos, k) {
    ...
    for (let dwarf of dwarfs) {
        ...
        switch (dwarf.gold) {
            case 1:
                canvas.drawImage(bufGold, -4 * 3 /* Inline.B_SCALE */, 70 /* Inline.groundLevel */ - 40);
                break;
            case 3:
                canvas.drawImage(bufGold, -8, 70 /* Inline.groundLevel */ - 48, 16, 18);
            case 2:
                canvas.drawImage(bufGold, -20, 70 /* Inline.groundLevel */ - 30, 16, 18);
                canvas.drawImage(bufGold, 4, 70 /* Inline.groundLevel */ - 30, 16, 18);
        }
        if (dwarf.turnBack)
            canvas.scale(-1, 1);
        canvas.drawImage(dwarf.buf(palette), -4 * 3 /* Inline.B_SCALE */, 3 /* Inline.B_SCALE */ * 11 * (1 - k) + dwarf.height, 3 /* Inline.B_SCALE */ * 8, 3 /* Inline.B_SCALE */ * 11 * k);
        canvas.restore();
    }
    return populated;
}
class Zone {
    render(t) {
        ...
        if (this.renderWaiting && dwarfsWaiting.length) {
            const count = Math.min(dwarfsWaiting.length + clearedForLanding, 24 /* Inline.WAITING_SIZE */);
            const buf = dwarfsWaiting[0].buf(this.palette);
            let pos, height;
            for (let n = count - 1; n >= clearedForLanding; --n) {
                if (n >= 9 /* Inline.WAITING_BOTTOM */) {
                    if (n >= 17 /* Inline.WAITING_SIZE_BM */) {
                        pos = 14 /* Inline.WAITING_TOP_POS */ + 3 /* Inline.B_SCALE */ *
                            (9 * (24 /* Inline.WAITING_SIZE */ - n) - 4);
                        height = 70 /* Inline.groundLevel */ - 46;
                    }
                    else {
                        pos = 0 /* Inline.WAITING_MIDDLE_POS */ + 3 /* Inline.B_SCALE */ *
                            (9 * (17 /* Inline.WAITING_SIZE_BM */ - n) - 4);
                        height = 70 /* Inline.groundLevel */ - 23;
                    }
                }
                else {
                    pos = -13 /* Inline.WAITING_BOTTOM_POS */ + 3 /* Inline.B_SCALE */ *
                        (9 * (9 /* Inline.WAITING_BOTTOM */ - n) - 4);
                    height = 70 /* Inline.groundLevel */;
                }
