...
let jaxpi = new Jaxpi({ name: "Gimli", mail: "gimli@lotr.com" }, 'http://localhost:3000/records', 'cEPTx-GsXov-dJBXe-pY7jc-NPyQ9');
...
function update(t) {
    ...
    for (let dwarf of dwarfs) {
        dwarf.advance();
        if (dwarf.purpose == 0 /* DwarfsPurpose.NONE */)
            dwarfsWaiting.push(dwarf);
        if (dwarf.pos > 0 && dwarf.pos < 160)
            clearedForLanding = 1;
        else if (dwarfAle &&
            dwarf.purpose != 3 /* DwarfsPurpose.ALE */ &&
            dwarf.pos > 250 && dwarf.pos < 670)
            dwarf.haveFun();
        else if (dwarfPortal && dwarf.pos > 200 && dwarf.pos < 720) {
            if (dwarf.purpose == 1 /* DwarfsPurpose.TREASURE */) {
                dwarf.prevPos = dwarf.pos = 720;
            }
            else if (dwarf.purpose == 2 /* DwarfsPurpose.FORTRESS */) {
    ...
    if (hasAutorun) {
        if (autorunWait >= autorunSpeed) {
            ...
            if (dwarfsWaiting.length)
                dwarfsWaiting[0].purpose = 1 /* DwarfsPurpose.TREASURE */;
        }
        else {
            autorunWaitPrev = autorunWait;
            autorunWait += t;
...
function orbital(done) {
    ...
    function renderOrbital() {
        const now = Date.now();
        if (now - start >= 2000 /* Inline.ORBITAL_DURATION */) {
            done();
            return;
        }
        ...
        renderDwarfs(1, forest.canvas, PAL_TREASURE, forest.pos, (2000 /* Inline.ORBITAL_DURATION */ + start - now) / 2000 /* Inline.ORBITAL_DURATION */);
    }
    requestAnimationFrame(renderOrbital);
}
